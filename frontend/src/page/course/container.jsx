// Imports for lazy-loaded components remain unchanged
...
const Container = () => {
  ...
  const filterBySubCategory = async (subCategory) => {
    const filtered = await selectedCourse.data.filter(course => course.category === subCategory);
  ...
  return (
    <div className="relative">
        <div className="bg-transparent w-5 absolute h-1 top-[-122px]" ref={ref}></div>
        ...
        <div className={`flex flex-col gap-2 w-64 min-h-full transition-all duration-300 ${isSticky ? "fixed top-[124px] w-52" : ""} ${isBottom ? "opacity-0" : "opacity-100"}`}>
            ...
            <div
                ...
                onClick={()=>{handleChangeMenu(course)}}
            >
                <Dropdown callbackFn={filterBySubCategory} heading={course.name} subCategories={course.subCategories}></Dropdown>
            </div>
            ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full mb-12 overflow-y-auto">
            {
                (filteredCourses.length > 0 ? filteredCourses : selectedCourse.data).map(courseData => {
                    return <CourseCard key={courseData.name} data={courseData}></CourseCard>
                })
            }
        </div>
        </div>
